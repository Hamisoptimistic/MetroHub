...
## 13. Windows OS Localization, Unicode & Invariant System Standards

> **CRITICAL ARCHITECTURAL DIRECTIVE**: MetroHub runs across international Windows installations (German, French, Spanish, Japanese, Chinese, Russian, etc.). Never write code that assumes Windows CLI tools output English text, and never use ASCII-only validation for system identifiers.

### 1. Unicode-Aware Regex for System Identifiers & Shell Safety
- **Anti-Pattern (Banned):** Using ASCII-only ranges like `^[a-zA-Z0-9\s\-_.#()]+$` to sanitize or validate adapter names, Wi-Fi SSIDs, device names, or user-configurable identifiers.
- **Failure Mode:** On non-English Windows, default adapter and connection names frequently contain non-ASCII Unicode characters (e.g. French `"Connexion réseau"`, German `"Drahtlosnetzwerk"`, Spanish `"Conexión de red"`, Russian `"Сетевой адаптер"`, or Japanese/Chinese network names). ASCII-only regex rejects these valid names, causing command validation failures and breaking adapter management.
- **Mandatory Practice:** Always use Unicode category classes (`\p{L}` for all Unicode letters across all human alphabets, `\p{N}` for all Unicode numbers):
  ```csharp
  // Allows international letters (\p{L}), numbers (\p{N}), spaces, and benign punctuation
  // while strictly rejecting shell injection metacharacters (&, |, ;, ", ', $, `, <, >, %, ^, \, \r, \n)
  private static readonly Regex ValidAdapterNameRegex = new(@"^[\p{L}\p{N}\s\-_.#()]+$", RegexOptions.Compiled);
  ```
- **Shell Metacharacter Blacklist:** Even when using Unicode classes, never permit shell metacharacters (`&`, `|`, `;`, `"`, `'`, `$`, `` ` ``, `<`, `>`, `%`, `^`, `\`, `\r`, `\n`) or command substitution sequences when executing elevated PowerShell or command-line processes.

### 2. Invariant Structural Parsing & Resilience Against Localized CLI Tools
- **Anti-Pattern (Banned):** Relying on strict English text matching (e.g. `parts[0].Equals("Enabled")`) when parsing output from Windows command-line utilities (`netsh.exe`, `ipconfig.exe`, `sc.exe`, `tasklist.exe`).
- **Variable-Length Win32 Tables Caution:** Native variable-length array structures in P/Invoke (such as `MIB_IF_TABLE2` via `GetIfTable2`) are susceptible to platform-specific 64-bit struct padding and alignment drift. Furthermore, NDIS exposes every intermediate filter driver (QoS Packet Schedulers, WFP Filters, Virtual Wi-Fi filters — often 40+ entries), which can overwhelm or obscure dedicated physical hardware adapters.
- **Mandatory Practice:**
  1. **Primary Enumeration via Managed Framework APIs:** Always query physical hardware interfaces first via managed .NET APIs (`NetworkInterface.GetAllNetworkInterfaces()`) with robust virtual adapter filtering.
